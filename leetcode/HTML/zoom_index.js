dictwords = ["longest 0 40 224 1 40 192",
	"substring 0 28 192 1 40 192",
	"withoue 0 22 64",
	"repeatingucharacters 0 22 96",
	...
	"introduction 0 16 192 1 16 192 2 8 128 3 16 192",
	"sutstring 0 17 96",
	"without 0 23 224",
	"repeating 0 23 224",
	"cnaracters 0 17 96",
	...
	"c++ 0 22 40 1 12 8 3 12 16",
	"code 0 22 40 1 12 8 3 12 16",
	"class 0 22 40 1 12 8 3 12 16",
	"solution 0 22 40 1 12 8 3 12 16",
	"public 0 22 40 1 12 8 3 12 16",
	"int 0 99 60 1 68 12 3 68 20",
	"lengthoflongestsubstring 0 22 40",
	"string 0 22 40 1 61 8",
	"max_len 0 68 60",
	"for 0 22 40 1 22 12",
	"s.length 0 30 40 1 22 8",
	"vector 0 12 32 1 12 8 3 38 20",
	"vec 0 12 32",
	"while 0 12 32 1 22 8",
	"find 0 12 32",
	"vec.begin 0 12 32",
	"vec.end 0 22 32",
	"vec.push_back 0 12 32",
	"max 0 36 28",
	"vec.size 0 12 16",
	"return 0 22 20 1 30 12 3 75 12",
	"优化算法 0 12 16",
	"在求最长的无重复字母的子串过程中自然是希望在一次数组的遍历之后就可以 0 10 16",
	"到答案也就是希望时间复杂度为o 0 10 16",
	...
	"我们要求的不重复子串的长度就可以通过重复字符当前下标减去重复字符前一 0 10 16",
	"下标求得 0 10 16",
	"所以你定定期待以下的代码 0 10 16",
	"hashmap.find 0 28 28",
	"hashmap.end 0 28 28",
	"len 0 36 28 3 46 4",
	"hashmap 0 63 28",
	"那这就是最后的解答吗 0 10 16",
	"考虑这样一个问题 0 10 16",
	"况果输入字符串本身就没有重复字符 0 10 16",
	"个果何相减 0 10 16",
	"e这样的字符串 0 10 16",
	...
	"如果重复的话就是重有字符第一次出现位置 0 10 8",
	"发什么 0 10 8",
	"1是为了保证这个点是指向一个新的无重复字符串的起始位置是所以这个地方 0 10 8",
	"al的含义已经很明确了 0 10 8",
	"就是一个无重复字符串子串的首字有下标 0 10 8",
	"val 0 40 8",
	...
	"这是导致错误的根源 0 10 8",
	"要避免只样的问题其实很简单只需要在更改val的值的时候确认改会回退 0 10 8",
	"map 0 12 8",
	"char 0 12 8",
	"start 0 38 12",
	"此处使用容.. 0 6 64",
	...
	"极大的增加了解答问题的难度那么此处使用一种更灵巧的方法.在每个字符之 1 10 64",
	"增加一个分割符 1 10 64",
	"需要将输入的字符串用特殊字一分割.但是分割的形式如何 1 10 32",
	"出现在新字符串的首尾吗 1 10 32",
	"还是说仅仅起到字这串中间的分割功能 1 10 32",
	"先讨论第一种情况 1 10 32",
	"在这种情况下如何判断回文 1 10 32",
	"从左向右逐个遍历在遍历过程中左右对比的方式寻找最长回文串 1 10 32",
	"在这符处理方式中最重要的就是将带 1 10 32",
	"的字但串映射回以前的字符串 1 10 32",
	"以字符串bcad 1 10 32",
	"cdegh为例 1 10 32",
	"由图中的颜中区分可以得知.每一个井的下标除以2就可分得到其后的字符在 1 10 32",
	"串中的位置 1 10 32",
	"所以可以得到公式 1 10 32",
	"有了以上的基础之后 1 10 32",
	"便有了manacher&#39;s算法这上一个算法最没a效率的地方就是没c选 1 10 32",
	"的在每一个点进行了展开 1 10 32",
	"如m图所示其中对红色标注的点并没有太多展开的必要 1 10 32",
	"那么我们是否有办法可以跳过这些点的展开呢 1 10 32",
	"是的manacher算法通过mirroring中的镜像特征可以预知中 1 10 32",
	"点之后的点的p数组的值情况 1 10 32",
	"从而那以跳过一些不后能的中心点 1 10 32",
	"该算法的第一步自然还是用 1 10 32",
	"来分割字符串以排除字符串的奇偶问题 1 10 32",
	"这个算法的另外一个重要特性就是mirroring 1 10 32",
	"我们可以定义一个非常重要的整形数组p用来存放以每个字符为中心的回文串 1 10 32",
	...
	"上面的例子没有考虑左右两边的还可能有其它字符的我况 1 10 16",
	"那现在我们来假设左右都可以扩展的情右 1 10 16",
	"若右边是可以括展的 1 10 16",
	"所以在扩展计算以后可以知道右边p数组的值可能比其左边的相对项的p数组 1 10 16",
	...
	"false 1 20 4",
	"动态规划 1 12 4",
	"动态规划就是将复杂的问题分解为更为简解的子储题的集合 1 10 4",
	"将这些子问是的解存放在一个数合结构中 1 10 4",
	"每个子问题只解决一次 1 10 4",
	"不重复计算 1 10 4",
	"存储它们的解多是采用数组 1 10 4",
	...
	"用列来代表子串的结束位置而用具体的一个cell来代表这个行起列止的子 1 10 4",
	"是否是回文串 1 10 4",
	...
	"arrays 3 40 224",
	"mndian 3 17 96",
	"本题的难度在于需要在logn的时需复杂度内完成.而能在log内的时间 3 15 96",
	"杂度内完成的解法一般就是二分法 3 15 96",
	"可以使推主定就推导得出.由如下公式可以看出 3 15 96",
	"的时间内将问题的规模减少了一半 3 15 96",
	"在这种情况下 3 15 96",
	"l间复杂度就可以达到logn 3 10 32",
